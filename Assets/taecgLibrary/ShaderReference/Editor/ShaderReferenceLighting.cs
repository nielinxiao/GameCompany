...
namespace taecg.tools.shaderReference
{
    public class ShaderReferenceLighting : EditorWindow
    {
        ...
        public void DrawMainGUI()
        {
            ...
            ShaderReferenceUtil.Instance.DrawTitle("法线 NormalMap");
            ShaderReferenceUtil.Instance.DrawOneContent("使用切线空间下的法线",
            ...
            ShaderReferenceUtil.Instance.DrawTitle("光照模型");
            ShaderReferenceUtil.Instance.DrawOneContent("Lambertian", "Diffuse = Ambient + Kd * LightColor * max(0,dot(N,L))\n" +
                "Diffuse:\t最终物体上的漫反射光强.\n" +
                ...
                "N:\t顶点的单位法线向量.\n" +
                "L:\t顶点指向光源的单位向量.");
            ShaderReferenceUtil.Instance.DrawOneContent("Phong", "Specular = SpecularColor * Ks * pow(max(0,dot(R,V)), Shininess)\n" +
            ...
            ShaderReferenceUtil.Instance.DrawOneContent("Blinn-Phong", "Specular = SpecularColor * Ks * pow(max(0,dot(N,H)), Shininess)\n" +
                "Specular:\t最终物体上的反射高光光强.\n" +
                "SpecularColor:\t反射光的颜色.\n" +
                "Ks:\t反射强度系数.\n" +
                "N:\t顶点的单位法线向量.\n" +
                ...
                "Shininess:\t乘方运算来模拟高光的变化.");
            ShaderReferenceUtil.Instance.DrawOneContent("Disney Principled BRDF", "f(l,v) = diffuse + D(h)F(v,h)G(l,v,h)/4cos(n·l)cos(n·v)\n" +
            ...
            switch (ShaderReferenceEditorWindow.mPipline)
            {
                case ShaderReferenceEditorWindow.Pipline.BuildIn:
                    ShaderReferenceUtil.Instance.DrawTitle("ShadowMap阴影");
                    ShaderReferenceUtil.Instance.DrawOneContent("生成阴影", "添加\"LightMode\" = \"ShadowCaster\"的Pass.\n" +
                    ...
                    ShaderReferenceUtil.Instance.DrawOneContent("采样阴影", "" +
                    ...
                    ShaderReferenceUtil.Instance.DrawTitle("Global Illumination 全局照明GI");
                    ShaderReferenceUtil.Instance.DrawOneContent("产生间接光照", "添加\"LightMode\" = \"Meta\"的Pass.\n" +
                        "可参考内置Shader中的Meta Pass.");

                    ShaderReferenceUtil.Instance.DrawTitle("Light Probe 光照探针");
                    ShaderReferenceUtil.Instance.DrawOneContent("规则01", "当逐像素平行灯标记为Mixed时,同时场景内有LightProbe时,那么当前平行灯的光照值会自动被LightProbe影响,所以不管物体Shader中是否有SH相关的运算,都会受到LightProbe的影响.");
                    ShaderReferenceUtil.Instance.DrawOneContent("规则02", "当逐像素平行灯标记为Baked时,同时场景内有LightProbe时,那么需要自行在物体Shader中添加SH相关的运算,才会受到LightProbe的影响.");

                    ShaderReferenceUtil.Instance.DrawTitle("Reflection Probe 反射探针");
                    ShaderReferenceUtil.Instance.DrawOneContent("反射探针的采样", "反射探针中当前激活的CubeMap存储在unity_SpecCube0当中，必须要用UNITY_SAMPLE_TEXCUBE进行采样，然后需要对其进行解码\n" +
                    ...
                    ShaderReferenceUtil.Instance.DrawTitle("Fog 雾效");
                    ShaderReferenceUtil.Instance.DrawOneContent("unity_FogColor", "内置雾效颜色值");
                    ShaderReferenceUtil.Instance.DrawOneContent("方案一:", "常规方案\n" +
                    ...
                    ShaderReferenceUtil.Instance.DrawOneContent("方案二:", "当在v2f中有定义worldPos时,可以把worldPos.w利用起来做为雾效值.\n" +
                        "1.#pragma multi_compile_fog声明雾效所需要的内置变体:FOG_LINEAR FOG_EXP FOG_EXP2.\n" +
                ...
                case ShaderReferenceEditorWindow.Pipline.URP:
                    ShaderReferenceUtil.Instance.DrawTitle("获取主平行灯");
                    ShaderReferenceUtil.Instance.DrawOneContent("Light light = GetMainLight();", "获取主平行灯相关参数:\n" +
                    ...
                    ShaderReferenceUtil.Instance.DrawTitle("Fog 雾效");
                    ShaderReferenceUtil.Instance.DrawOneContent("unity_FogColor", "内置雾效颜色值");
                    ShaderReferenceUtil.Instance.DrawOneContent("实现方法", "#pragma multi_compile_fog\n" +
                    ...
                    ShaderReferenceUtil.Instance.DrawOneContent("Linear", "线性雾公式:(end-z)/(end-start)");
                    ShaderReferenceUtil.Instance.DrawOneContent("EXP", "指数雾公式:exp(-density*z)");
                    ShaderReferenceUtil.Instance.DrawOneContent("EXP2", "指数2次方雾公式:exp(-(density*z)^2)");
                    break;
            }
